...
result1 <- matrix(0, nrow = 100, ncol = 5)
result2 <- matrix(0, nrow = 100, ncol = 5)
...
column_names <- c("SumScore", "SEM", "SAM", "RGCCA", "LSLV")
colnames(result1) <- column_names
colnames(result2) <- column_names

save(result1, result2, file = "RealData.RData")
...
boxplot(result1, main = "Error 1", ylab = "Error", xlab = "Method", outline=FALSE)
boxplot(result2, main = "Error 2", ylab = "Error", xlab = "Method", outline=FALSE)


# anova
# Data preparation
error_matrix <- t(result2)
rownames(error_matrix) <- c("sumscore", "sem", "sam", "rgcca", "lslv")
error_data <- data.frame(
  Method = rep(rownames(error_matrix), each = 100),
  Error = as.vector(t(error_matrix))
)
error_data$Method <- factor(error_data$Method, levels = c("sumscore", "sem", "sam", "rgcca", "lslv"))
